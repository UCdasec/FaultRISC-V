...
int main() {
    ...
    // Store the result of isMaintenanceRequired function in a variable
    int isMaintenanceRequiredResult = isMaintenanceRequired(operatingHours);

    // Check the stored result of isMaintenanceRequired function
    if (isMaintenanceRequiredResult) {
        performMaintenance();
    } else {
        // Check motor status
